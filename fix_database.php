...
try {
    ...
    echo "<h1>Database Customization Tool</h1>";
    echo "<h2>Applying Real Stock Photos... 📸✨</h2>";
    ...
    $pdo->exec($sql_create);
    echo "<p style='color: blue;'>Step 1: Products table reset.</p>";

    // 2. Insert Data with LoremFlickr Real URLs
    // URL Format: https://loremflickr.com/400/400/{keywords}?lock={id}
    // Using 'lock' ensures the image stays the same for that product ID and doesn't change on reload.
    ...
    $pdo->exec($sql_insert);
    echo "<p style='color: blue;'>Step 2: Real Stock Photos inserted.</p>";

    // 3. Verify
    $count = $pdo->query("SELECT COUNT(*) FROM products")->fetchColumn();
    echo "<h2 style='color: green;'>Success! " . $count . " Products with Real Images Added</h2>";
    
    $pdo->exec("SET FOREIGN_KEY_CHECKS=1");
    
    echo "<a href='index.php' style='display: inline-block; background: #ff5fa2; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-top: 20px;'>Go to Home Page</a>";
    echo "</body></html>";
